...
function parseCity(type, prov) {
    ...
    result.push({value: '', text: '请选择', selected: true});
    for (let i in cities) {
        result.push({value: cities[i], text:cities[i], selected: false})
    }
    return result;
}
...
const a2 = new Select({
    ...
    const prov = target.attr('data-value');
    const model = parseCity(a1.get('value'), prov);
    a3.syncModel(model);
});
