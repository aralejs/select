...
var _araleOverlay = require('arale-overlay');

var _araleOverlay2 = _interopRequireDefault(_araleOverlay);

var _araleTemplatable = require('arale-templatable');

var _araleTemplatable2 = _interopRequireDefault(_araleTemplatable);
...
var Select = _araleOverlay2.default.extend({
    ...
    attrs: {
        trigger: {
            ...
            getter: function getter(val) {
                return (0, _jquery2.default)(val).eq(0);
            }
        },
    ...
    events: {
        ...
        'click [data-role=item]': function clickDataRoleItem(e) {
            var target = (0, _jquery2.default)(e.currentTarget);
            if (!target.data('disabled')) {
                this.select(target);
            }
        },
        'mouseenter [data-role=item]': function mouseenterDataRoleItem(e) {
            var target = (0, _jquery2.default)(e.currentTarget);
            if (!target.data('disabled')) {
                target.addClass(getClassName(this.get('classPrefix'), 'hover'));
            }
        },
        'mouseleave [data-role=item]': function mouseleaveDataRoleItem(e) {
            var target = (0, _jquery2.default)(e.currentTarget);
            if (!target.data('disabled')) {
    ...
    initAttrs: function initAttrs(config, dataAttrsConfig) {
        ...
        if (trigger[0].tagName.toLowerCase() === 'select') {
            ...
            // 替换 trigger
            var newTrigger = (0, _jquery2.default)(this.get('triggerTpl')).addClass(getClassName(this.get('classPrefix'), 'trigger'));
            ...
            if (selectName) {
                var input = (0, _jquery2.default)('input[name="' + selectName + '"]').eq(0);
                if (!input[0]) {
                    input = (0, _jquery2.default)('<input type="text" tabindex="-1" id="select-' + selectName.replace(/\./g, '-') + '" name="' + selectName + '" />').css({
                        position: 'absolute',
                        left: '-99999px',
                        zIndex: -100
                    }).insertAfter(trigger);
    ...
    syncModel: function syncModel(model) {
    ...
    enableOption: function enableOption(option) {
        var index = getOptionIndex(option, this.options);
        var model = this.get("model").options;
    ...
    disableOption: function disableOption(option) {
        var index = getOptionIndex(option, this.options);
        var model = this.get("model").options;
    ...
    _onRenderSelectedIndex: function _onRenderSelectedIndex(index) {
        if (index === -1) return;
    ...
    _onRenderDisabled: function _onRenderDisabled(val) {
    ...
    // trigger 的宽度和浮层保持一致
    _setTriggerWidth: function _setTriggerWidth() {
        var trigger = this.get('trigger');
    ...
    _tweakAlignDefaultValue: function _tweakAlignDefaultValue() {
    ...
    _triggerHandle: function _triggerHandle(e) {
        e.preventDefault();
    ...
    _initHeight: function _initHeight() {
...
function completeModel(model, classPrefix) {
    ...
    for (i = 0, l = model.length; i < l; i++) {
        var o = _jquery2.default.extend({}, model[i]);
...
function getOptionIndex(option, options) {
    var index;
    if (_jquery2.default.isNumeric(option)) {
        // 如果是索引
        index = option;
...
function syncSelect(select, model) {
    ...
    if (select.tagName.toLowerCase() === 'select') {
        (0, _jquery2.default)(select).find('option').remove();
        for (var i in model) {
            var m = model[i];
...
function getLiHeight(ul) {
    ...
    ul.find('li').each(function (index, item) {
        height += (0, _jquery2.default)(item).outerHeight();
    });
    return height;
}